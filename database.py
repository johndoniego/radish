...
def update_schedule(entry_id, room, date, subject, start_hour, start_minute, start_period, end_hour, end_minute, end_period, class_name, teacher_id):
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("""
            UPDATE schedule
            ...
            WHERE id = ?
        """, (room, date, subject, start_hour, start_minute, start_period, end_hour, end_minute, end_period, class_name, teacher_id, entry_id))
        conn.commit()
