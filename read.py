...
class ScheduleViewerApp:
    ...
    def create_sheet(self):
        sheet = Sheet(self.root, headers=["ID", "Room", "Date", "Subject", "Start Time", "End Time", "Class Name", "Teacher"])
        sheet.enable_bindings(("single_select", "row_select", "column_select", "arrowkeys", "right_click_popup_menu", "rc_select", "rc_insert_row", "rc_delete_row"))
        sheet.pack(fill=tk.BOTH, expand=True)
        sheet.bind("<Double-1>", self.on_double_click)
        
        # Hide the ID column
        sheet.column_width(0, width=0)
    ...
    def load_schedule(self, schedule):
        self.sheet.set_sheet_data([[entry["id"], entry["room"], entry["date"], entry["subject"], f"{entry['start_hour']}:{entry['start_minute']} {entry['start_period']}", f"{entry['end_hour']}:{entry['end_minute']} {entry['end_period']}", entry["class_name"], entry["teacher_name"]] for entry in schedule])
    ...
    def on_double_click(self, event):
        ...
        values = self.sheet.get_row_data(row)
        if values[7] != get_teacher_name(self.teacher_id):
            messagebox.showerror("Error", "You can only edit your own schedule.")
            return
        if col == 1:  # Room column
            new_value = self.get_room_dropdown(values[col])
        elif col == 2:  # Date column
            new_value = self.get_date(values[col])
        elif col == 7:  # Teacher column
            messagebox.showerror("Error", "You cannot edit the teacher.")
            return
        else:
            new_value = simpledialog.askstring("Input", f"Enter new value for {self.sheet.headers()[col]}:", initialvalue=values[col])
        if new_value is not None:
            self.undo_stack.append(("edit", values))
            self.redo_stack.clear()  # Clear the redo stack on new action
            values[col] = new_value
            room, date, subject, start_time, end_time, class_name, teacher_name = values[1:8]
    ...
    def save_schedule(self):
        ...
        for entry in self.modified_schedule:
            ...
            update_schedule(entry_id, room, date, subject, start_hour, start_minute, start_period, end_hour, end_minute, end_period, class_name, teacher_id)
        self.checkpoint_schedule = self.modified_schedule.copy()  # Save the current state as the checkpoint
        self.load_teacher_schedule(self.teacher_id)  # Reload the schedule to reflect changes
        print("Schedule saved.")

    def remove_row(self):
        selected_item = self.sheet.get_currently_selected()
        if selected_item:
            row = selected_item[0]
            values = self.sheet.get_row_data(row)
            if values[7] != get_teacher_name(self.teacher_id):
